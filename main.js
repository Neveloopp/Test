...
if (!text) {
await sock.sendMessage(msg.key.remoteJid, {
text: `✳️ Usa el comando correctamente:\n\n📌 Ejemplo:\n• *${global.prefix}latanime* Gachiakuta`
}, { quoted: msg });
break;
}
...
try {
const apiURL = `https://neveloopp-api.vercel.app/api/download/latanime?query=${encodeURIComponent(text)}`;
const res = await axios.get(apiURL);
const data = res.data;
...
case 'playanime': {
    ...
    if (!text) {
        await sock.sendMessage(msg.key.remoteJid, {
            text: `✳️ Usa el comando correctamente:\n\n📌 Ejemplo:\n• *${global.prefix}playanime* Gachiakuta 1`
        }, { quoted: msg });
        break;
    }
    ...
    if (!match) {
        await sock.sendMessage(msg.key.remoteJid, {
            text: `❌ Debes especificar el capítulo.\n\n📌 Ejemplo:\n• *${global.prefix}playanime* Gachiakuta 1`
        }, { quoted: msg });
        break;
    }
    ...
    try {
        const apiURL = `https://neveloopp-api.vercel.app/api/download/latanime?query=${encodeURIComponent(animeName)}`;
        const res = await axios.get(apiURL);
        const data = res.data;

        if (!data.results || !data.results.episodes || !data.results.episodes.length) {
            await sock.sendMessage(msg.key.remoteJid, {
                text: `❌ No se encontraron resultados para: *${animeName}*`
            }, { quoted: msg });
            break;
        }

        const anime = data.results;
        const ep = anime.episodes.find(e => parseInt(e.episodio) === episodeNumber);

        if (!ep || !ep.PixelLink) {
            await sock.sendMessage(msg.key.remoteJid, {
                text: `❌ No se encontró el capítulo *${episodeNumber}* de *${anime.title}*`
            }, { quoted: msg });
            break;
        }
